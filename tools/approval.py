...
def check_dangerous_command(command: str, env_type: str,
    ...
    #
    # /yolo remains available — session-scoped (gateway) or process-scoped (CLI).
    # Director opted in 2026-04-12 (leave yolo on).
...
def check_all_command_guards(command: str, env_type: str,
    ...
    """
    # SENTINEL SECURITY FIX (3.1): Docker approval bypass REMOVED.
    # SENTINEL SECURITY FIX (3.3): approvals.mode="off" bypass REMOVED — config must not collapse all safety.
    # /yolo remains available — session-scoped (gateway) or process-scoped (CLI).
    # Director opted in 2026-04-12 (leave yolo on).
